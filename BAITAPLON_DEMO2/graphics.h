...
struct Sprite {
    ...
    void attack(Sprite& object, SDL_Event e){
        ...
        if(e.type==SDL_MOUSEBUTTONDOWN&&khoangcachx>=0 &&khoangcachx <= 80+10 && khoangcachy<= 40)
        {
...
struct Graphics {
    ...
    void DoPlayer(float &x, float &y ,const Sprite& sprite, const Uint8* currentKeyStates, float morespeedrun){
        ...
        if((currentKeyStates[SDL_SCANCODE_UP]||currentKeyStates[SDL_SCANCODE_SPACE]) && y==340)
        {
            y_val = -jump;
        }
        if(currentKeyStates[SDL_SCANCODE_W] && y==340)
        {
            y_val = -(jump-jump/2.7);
        }
    ...
    void move(float &x, float &y ,const Sprite& sprite)
    {
        ...
        y+=y_val;
        if (x < -10) {
            x = -10; // Đảm bảo không vượt quá biên trái
        }
        if (y < 0) {
     ...
     void movebird(float &x, float &y , Sprite& sprite, float more_speed)
    {
        ...
        if(x>=SCREEN_WIDTH) y=(rand()%100)+240;
        const SDL_Rect* clip = sprite.getCurrentClip();
        SDL_Rect renderQuad = {x, y, clip->w, clip->h};
        SDL_RenderCopy(renderer, sprite.texture, clip, &renderQuad);
    }
     ...
     void movefatbird(float &x, float &y , Sprite& sprite, float more_speed, Sprite& hero)
    {
        if(x>hero.x){
    ...
    bool gameOverFatBird(float &x, float &y, float& x_bird, float &y_bird )
    {
        float left1 = x;
        float right1 = x + 58;
        float top1 = y;
        float bottom1 = y + 50;
        float left2 = x_bird;
        float right2 = x_bird + 40;
        float top2 = y_bird;
        float bottom2 = y_bird + 22;
